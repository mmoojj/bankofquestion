...
import 'package:bankofquestion_fix/ui/widgets/customappbarhomepage.dart';
import 'package:flutter/material.dart';

class Quize extends StatefulWidget {
  @override
  State<StatefulWidget> createState() => QuizeState();
}

class QuizeState extends State<Quize> {
  ...
  void _onsubmitted(String text) async {
    if (await checkconnection()) {
      List<QuizeModel> list =
          await UserService().getquizeforuseranaymos(searchitem: text);
      setState(() {
        listdata = list;
      });
    } else {
  ...
  Future<void> _refresh() async {
    if (await checkconnection()) {
      return UserService().getquizeforuseranaymos().then((value) {
        setState(() {
          listdata = value;
        });
      });
  ...
  void initiallist() async {
    if (await checkconnection()) {
      UserService().getquizeforuseranaymos().then((value) {
        setState(() {
          listdata = value;
        });
      });
